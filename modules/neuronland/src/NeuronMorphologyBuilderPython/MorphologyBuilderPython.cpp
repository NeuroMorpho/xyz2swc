...
/*
	Would prefer to auto-generate this from a specification file (or tagged C++)
	Could then auto-generate correct binding code for any scripting language which 
	might be supported.
*/

BOOST_PYTHON_MODULE(MorphologyBuilder)
{
	class_<SamplePoint>("SamplePoint", init<>())
	...
	class_<MarkerPoint, bases<SamplePoint> >("MarkerPoint", init<>())
		.def(init<float, float, float, float>());

	class_<Neuron3D>("Neuron3D", init<>());

	class_<MorphologyBuilder::Soma>("Soma", no_init)
	...
	class_<MorphologyBuilder::Dendrite>("Dendrite", no_init)
		.def("GetRoot",						&MorphologyBuilder::Dendrite::GetRoot);

	class_<MorphologyBuilder::Axon>("Axon", no_init)
		.def("GetRoot",						&MorphologyBuilder::Axon::GetRoot);

	class_<MorphologyBuilder::Branch>("Branch", no_init)
		...
		.def(self <=	other<MorphologyBuilder::Branch>())
		.def(self <	other<MorphologyBuilder::Branch>())
		.def(self ==	other<MorphologyBuilder::Branch>());

	class_<MorphologyBuilder>("MorphologyBuilder", init<Neuron3D&>())
...
{
	//PyImport_AppendInittab("MorphologyBuilder", initMorphologyBuilder);
	//PyImport_AddModule("MorphologyBuilder");

	if (PyImport_AppendInittab("MorphologyBuilder", initMorphologyBuilder) == -1)
		throw std::runtime_error("Failed to add MorphologyBuilder to the interpreter's " "builtin modules");
}
