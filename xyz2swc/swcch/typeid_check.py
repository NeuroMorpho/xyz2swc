...
# ------------------------------------------------------
# TypeID check
def typeid_check(swc_matrix, soma_rows, tree_rows, check_list_df, Lines, commentline_numbers, do_NStudio=True, do_swcPlus=True):
    nonstandard_somaID = None
    swcPlus_remove = None
    ...
    # NonStandard TypeID
    if all(swc_matrix["TypeID"] >= 0) and all(swc_matrix["TypeID"] <= 7):
        if do_NStudio:
        ...
        if not (nonstandard_somaID is None):
            temp_comment = "ERROR: Detected Soma Type=" + str(nonstandard_somaID) + ", must be 1."
            check_list_df.loc[5, ["Value", "Status", "ErrorMsg"]] = [True, "FAIL", temp_comment]
        else:
            if is_swcPlus_flag:
                check_list_df.loc[5, ["Value", "Status", "ErrorMsg"]] = [True, "PASS", "Warning: SWC+ File Detected"]
            elif any(swc_matrix["TypeID"] < 0):  # Negative TypeIDs
                check_list_df.loc[5, ["Value", "Status", "ErrorMsg"]] = [True, "PASS", "Warning: Type cannot be negative."]
                unspecified_typeid_flag = True
            else:
                check_list_df.loc[5, ["Value", "Status", "ErrorMsg"]] = [True, "PASS", "Warning: Type must be a positive integer value"]
                unspecified_typeid_flag = True
