...
from .swcco import contour_convert, swc_sort, set_unspecified_typeID, set_standard_somaID, set_default_radius, remove_bifuc_neuronstudio, set_invalidparent_to_root, cut_spiderlegs, correct_invalid_xyz,break_loop
...
def chksingle(filename, outfolder=None, verbose=True, write_csv=True):
    ...
    conversion_flags = con_flags_tuple(stop_conversion_flag, convert_contour_flag, set_root_flag, invalidparent_to_root_flag, reset_index_flag, do_sort_flag, set_defaultradius_flag, unspecified_typeid_flag, nonstandard_somaID, swcPlus_remove, remove_bifucNS_flag, cut_spiderlegs_flag, invalid_XYZ_flag)

    check_attribute_namelist = ['Missing Field Columns', 'No. of tree samples', 'No. of soma samples',
                                'Root ParentIndex', 'Invalid ParentIndex',
                                'NonStandard TypeID',
                                'ITP int', 'XYZ double', 'Radius positive double',
                                'Index sequential', 'Sorted Index Order',
                                'Contour', 'Abnormal Compartments']

    check_list_df = pd.DataFrame({'Check': check_attribute_namelist,
                                  'Value': [' '] * len(check_attribute_namelist),
                                  'Status': [' '] * len(check_attribute_namelist),
                                  'ErrorMsg': [' '] * len(check_attribute_namelist)})
    ...
    check_list_df, stop_check_flag, unspecified_typeid_flag, nonstandard_somaID, swcPlus_remove, remove_bifucNS_flag = typeid_check.typeid_check(swc_matrix, soma_rows, tree_rows, check_list_df, Lines, commentline_numbers, do_NStudio, do_swcPlus)
    ...
    if write_csv:
        ...
        check_list_df.to_csv(os.path.join(outfolder, csv_outfile),
                             index=None, sep=',', mode='w')

        # np.savetxt('./output/new_file.swc', updated_swc_matrix.values, fmt="%d %d %f %f %f %f %d")
        # updated_swc_matrix.to_csv('./output/new_file_v2.ssort_matrix
        #                           mode='w')
...
def swccorrect(filename, check_list_df, conversion_flags, filename_corrected=None, spider_leg_type=0, verbose=True, write_csv=True):
    ...
    if not(conversion_flags.somaID is None) or not(conversion_flags.swcPlusCustom is None):
        ...
        correction_list_df.loc[5, "Correction"] = "Soma TypeID set to 1."

    if (conversion_flags.typeID):
        updated_swc_matrix = set_unspecified_typeID.set_unspecified_typeID(updated_swc_matrix)
        correction_list_df.loc[5, "Correction"] = "Nonstandard TypeIDs converted into TypeID=6 indicating Unspecified Neurite."
    ...
    if (conversion_flags.invalidparent is True):
        ...
        if(nochange_flag is True):
            ...
            correction_list_df.loc[4, "Correction"] = "Set invalid ParentIndex to -1."

    if (conversion_flags.bifucNS):
        updated_swc_matrix = remove_bifuc_neuronstudio.remove_bifuc_neuronstudio(updated_swc_matrix)
        temp_string = correction_list_df.loc[5, "Correction"]
        if temp_string.isspace():
            correction_list_df.loc[5, "Correction"] = "Set bifurcation TypeID to that of branch, and terminal TypeID to that of parent."
        else:
            correction_list_df.loc[5, "Correction"] = temp_string + " Set bifurcation TypeID to that of branch, and terminal TypeID to that of parent."
    ...
    # save corrected swc
    np.savetxt(filename_corrected, updated_swc_matrix, fmt="%u %u %f %f %f %f %d", header=hdr)

    if write_csv:
        csv_outfile = os.path.basename(filename).split('.')[0] + '_correctionlist.csv'
        correction_list_df.to_csv(os.path.join(outfolder, csv_outfile),
                                  index=None, sep=',', mode='w')
